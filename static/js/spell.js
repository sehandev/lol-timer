...
function set_fix_cooldown(index) {
    document.getElementById('fix-cool-' + summoner_array[index].index).innerHTML = check_fix_cool_range(summoner_array[index].fix_cool);
...
function init() {
    ...
    for (let i = 0; i < 5; i++) {
        let summoner = {
            index: i + 1,
            ult: 70, // 궁극기 재사용대기시간
            ult_time: 70, // 궁극기 남은 시간
            spellD: 360, // D spell 재사용대기시간
            spellD_time: 360, // D spell 남은 시간
            spellF: 320, // F spell 재사용대기시간
            spellF_time: 320, // F spell 남은 시간
